...
public class AdminResumeController(IAdminResumeService adminResumeService) : BaseController
{
    ...
    public async Task<IActionResult> GetResume(Guid studentId) =>
        HandleResult(await adminResumeService.GetResumeAsync(studentId));

    [HttpPut("{studentId:guid}")]
    public async Task<IActionResult> UpdateResume(Guid studentId, [FromBody] ResumeFillDto dto) =>
    ...
    public async Task<IActionResult> SoftDeleteResume(Guid studentId) =>
        HandleResult(await adminResumeService.SoftDeleteResumeAsync(studentId));

    [HttpDelete("{studentId:guid}/hard")]
    public async Task<IActionResult> HardDeleteResume(Guid studentId) =>
